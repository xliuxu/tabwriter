...
// ANSI escape sequences start
const EscapeAscii = '\x1b'

// CSI (Control Sequence Introducer) Start
// https://en.wikipedia.org/wiki/ANSI_escape_code
const EscapeCSIStart = '['

//SGR (Select Graphic Rendition) End
const EscapeSGREnd = 'm'
...
func (b *Writer) startEscape(ch byte) {
	...
	case Escape:
		b.endChar = Escape
	case EscapeCSIStart:
		b.endChar = EscapeCSIStart
	case EscapeSGREnd:
		b.endChar = EscapeSGREnd
	case '<':
		b.endChar = '>'
	case '&':
		b.endChar = ';'
...
func (b *Writer) endEscape() {
	...
	case EscapeSGREnd: // sgr counted as zero length
	case ';':
...
func (b *Writer) Write(buf []byte) (n int, err error) {
	...
	for i, ch := range buf {
		if b.endChar == 0 {
			...
			case EscapeAscii: //Escape SGR (Select Graphic Rendition) chars. Only "CSI n m" eg. \e[40;38;5;82m is supported
				b.append(buf[n:i])
				b.updateWidth()
				n = i
				b.startEscape(EscapeCSIStart)
			case Escape:
			...
			if b.endChar == EscapeCSIStart {
				if ch != EscapeCSIStart { // usupported escape sequences. stop escaping.
					b.endChar = 0
					j := i + 1
					b.append(buf[n:j])
					b.updateWidth()
					n = j
				} else { // EscapeCSIStart found
					b.endChar = EscapeSGREnd
				}
